...
class ProductCard extends React.Component {
  ...
  quickAddToCart(e) {
    e.preventDefault()
    const selectedAttributes = this.props.product.attributes.reduce((acc, attr) => {
  ...
  render() {
    ...
    return (
      <Link
        ...
        className="h-[444px] w-[388px] p-4 hover:cursor-pointer hover:shadow-xl"
      >
        <div className="group flex h-full w-full flex-col justify-between">
          <div className="relative h-[350px]">
            <div className="relative h-full w-full">
              <img
                ...
                alt="product"
                className={twMerge('h-full w-full object-cover', product.inStock || 'opacity-50')}
              />
              {product.inStock === false && (
                <h1 className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-2xl uppercase text-gray-300">
                  Out of stock
                </h1>
              )}
            </div>
            <button
              onClick={(e) => this.quickAddToCart(e)}
              className={twMerge(
                `absolute -bottom-5 right-5 z-10 hidden rounded-full bg-green-400 p-3`,
                product.inStock && 'hover:shadow-xl group-hover:flex',
              )}
            >
              <svg
                width="22"
          ...
          <div>
            <h3>{product.name}</h3>
            <p className={twMerge(`font-semibold`, product.inStock === false && 'text-gray-300')}>
